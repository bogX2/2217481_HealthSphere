...
const DoctorVerification = () => {
    ...
    const fetchPendingDoctors = async () => {
        ...
        try {
            ...
            // Ora facciamo la fetch dei dati utente per ogni medico
            const enrichedDoctors = await Promise.all(res.data.doctors.map(async doc => {
                try {
                    ...
                    return { ...doc, profile: userRes.data };
                } catch {
                    return doc; // fallback se la fetch fallisce
                }
            }));
    ...
    const handleVerify = async (doctorId, action) => {
        ...
        try {
            ...
            // Aggiorna lo stato locale dei medici
    ...
    // Redirect se non loggato o non admin
    if (!user && !loadingUser) {
        navigate('/');
        return null;
    }
        ...
        <div className="d-flex flex-column bg-light min-vh-100">
            ...
            <div className="container mt-5 pt-5 pb-5">
                <section className="mb-5 text-center pt-5">
                    <h2 className="display-5 fw-bold text-success">Medici in attesa di verifica</h2>
                    {error && <p className="text-danger">{error}</p>}
                </section>

                {doctors.length === 0 ? (
                    <p className="text-center text-muted">Nessun medico in attesa</p>
                ) : (
